		 ...
		 Else: The method fails with a diagnostic error code.*/
{
  ...
  delete useless;
  if (count == 1 && position == 0) current = NULL;
  count--;
  return Error_code::success;
}       
		 ...
		 Else: The method fails with a diagnostic error code*/
{
  ...
  if (position < 0 || position > count)
	return Error_code::range_error;
  if (position == 0) {
	if (count == 0) preceding = following = NULL;
	else {
	  set_position(0);
	  following = current;
...
/* Post: The List is initialized as a copy of original.*/
{
  ...
  Node <List_entry>* new_node, * first_node, * original_node = original.current;
  current_position = 0;
  if (original_node == NULL)
	...
	first_node = current;
	step++;
	while (original_node->next != original.current) {
